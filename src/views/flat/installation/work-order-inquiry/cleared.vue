...
<script>
import { fetchListCleared, exportListCleared } from '@/api/installation'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchListCleared(this.listQuery).then(response => {
    ...
    async handleDownload() {
      ...
      try {
        ...
        const response = await exportListCleared(queryForAll)
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          excel.export_json_to_excel({
            header: tHeader,
            data,
            filename: `装机工单(清零)-${new Date().toLocaleString()}`,
            autoWidth: true,
            bookType: 'xlsx'
          })
        })
