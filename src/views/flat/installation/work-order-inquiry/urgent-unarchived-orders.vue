...
<script>
import { fetchListUrgentUnarchivedOrders, exportListUrgentUnarchivedOrders } from '@/api/installation'
...
export default {
  ...
  data() {
    return {
      ...
      filterConfig: [
        ...
        { key: 'yidong_grid', placeholder: '网格', type: 'input' },
      ...
      tableColumns: [
        ...
        // 以下字段来自 installation_accumulated a
        ...
        // 以下字段来自 technician c
        { prop: 'technician_name', label: '装维姓名', width: '110', align: 'center' },
        { prop: 'technician_phone', label: '装维电话', width: '110', align: 'center' },
        { prop: 'yidong_grid', label: '移动网格', width: '110', align: 'center' },
        { prop: 'tietong_grid', label: '铁通网格', width: '110', align: 'center' },
        // 以下字段来自 complaint b
        { prop: 'formno', label: '投诉关联-工单号', width: '110', align: 'center' },
        { prop: 'hrstate', label: '投诉关联-工单状态', width: '110', align: 'center' },
        { prop: 'starttime', label: '投诉关联-派单时间', width: '110', align: 'center' },
        { prop: 'dwzname', label: '投诉关联-代维站名称', width: '110', align: 'center' },
        { prop: 'jibie', label: '投诉关联-全球通标识', width: '110', align: 'center' },
        { prop: 'title', label: '投诉关联-工单主题', width: '110', align: 'center' },
        // { prop: 'faultphenomenon', label: '投诉关联-故障现象', width: '110', align: 'center' },
        { prop: 'tprosecute', label: '投诉关联-投诉细类', width: '110', align: 'center' },
        { prop: 't1dealtime', label: '投诉关联-T1环节处理时间', width: '110', align: 'center' },
        { prop: 't3dealtime', label: '投诉关联-T3环节处理时间', width: '110', align: 'center' },
        { prop: 't4dealtime', label: '投诉关联-T4环节处理时间', width: '110', align: 'center' },
        { prop: 't1owername', label: '投诉关联-T1处理人', width: '110', align: 'center' },
        { prop: 't1cellphone', label: '投诉关联-T1处理人电话', width: '110', align: 'center' },
        { prop: 'nowperson', label: '投诉关联-当前代办人', width: '110', align: 'center' },
        { prop: 'insertappointmentime', label: '投诉关联-上门打卡时间', width: '110', align: 'center' },
        { prop: 'cssnserialno', label: '投诉关联-客服侧工单流水号', width: '110', align: 'center' },
        { prop: 'faultworkno', label: '投诉关联-群障关怀关联故障工单', width: '110', align: 'center' },
        { prop: 'oltname', label: '投诉关联-关联OLT名称', width: '110', align: 'center' },
        { prop: 'ponname', label: '投诉关联-关联PON名称', width: '110', align: 'center' }
      ],
      statusMap: {
        '2': '运行中',
        '3': '已归档'
  ...
  methods: {
    getList() {
      ...
      fetchListUrgentUnarchivedOrders(this.listQuery).then(response => {
    ...
    async handleDownload() {
      ...
      try {
        ...
        const response = await exportListUrgentUnarchivedOrders(queryForAll)
        ...
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['区县', '工单编号', '工单标题', '工单状态', '派单时间', '归档时间', '业务受理时间', '客户期望安装时间', '最后一次预约时间', '宽带账号', '小区ID', '小区名称', '装维账号', '装维姓名', '装维电话', '移动网格', '铁通网格', '投诉关联-工单号', '投诉关联-工单状态', '投诉关联-派单时间', '投诉关联-代维站名称', '投诉关联-全球通标识', '投诉关联-工单主题', '投诉关联-投诉细类', '投诉关联-T1环节处理时间', '投诉关联-T3环节处理时间', '投诉关联-T4环节处理时间', '投诉关联-T1处理人', '投诉关联-T1处理人电话', '投诉关联-当前代办人', '投诉关联-上门打卡时间', '投诉关联-客服侧工单流水号', '投诉关联-群障关怀关联故障工单', '投诉关联-关联OLT名称', '投诉关联-关联PON名称']
          const filterVal = ['county', 'form_no', 'form_title', 'current_state', 'send_time', 'end_time', 'ywsl_date', 'custom_expect_time', 'latest_expect_time', 'broadband_account', 'zone_id', 'zone_name', 'technician_id', 'technician_name', 'technician_phone', 'yidong_grid', 'tietong_grid', 'formno', 'hrstate', 'starttime', 'dwzname', 'jibie', 'title', 'tprosecute', 't1dealtime', 't3dealtime', 't4dealtime', 't1owername', 't1cellphone', 'nowperson', 'insertappointmentime', 'cssnserialno', 'faultworkno', 'oltname', 'ponname']
          const data = this.formatJson(filterVal, allData)
          excel.export_json_to_excel({
            header: tHeader,
            data,
            filename: `催装工单(未归档)-${new Date().toLocaleString()}`,
            autoWidth: true,
            bookType: 'xlsx'
          })
        })
