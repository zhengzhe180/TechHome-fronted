...
<script>
import { fetchListTomorrow, exportListTomorrow } from '@/api/installation'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchListTomorrow(this.listQuery).then(response => {
    ...
    async handleDownload() {
      ...
      try {
        ...
        const response = await exportListTomorrow(queryForAll)
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          excel.export_json_to_excel({
            header: tHeader,
            data,
            filename: `装机工单-${parseTime(new Date(new Date().getTime() + 24 * 60 * 60 * 1000), '{y}-{m}-{d}')}`,
            autoWidth: true,
            bookType: 'xlsx'
          })
        })
