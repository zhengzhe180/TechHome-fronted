...
<script>
import { fetchListWordCui, exportListWordCui } from '@/api/installation'
...
export default {
  ...
  methods: {
    getList() {
      ...
      fetchListWordCui(this.listQuery).then(response => {
    ...
    async handleDownload() {
      ...
      try {
        ...
        const response = await exportListWordCui(queryForAll)
        ...
        import('@/vendor/Export2Excel').then(excel => {
          ...
          excel.export_json_to_excel({
            header: tHeader,
            data,
            filename: `催字工单(累计)-${new Date().toLocaleString()}`,
            autoWidth: true,
            bookType: 'xlsx'
          })
        })
